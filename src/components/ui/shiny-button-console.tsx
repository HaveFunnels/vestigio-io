...
// ──────────────────────────────────────────────
// ShinyButtonConsole — Framer Motion sweep variant
//
// Split out from shiny-button.tsx so framer-motion (~30KB gzip)
// only loads on routes that actually use the console variant
// (i.e. /app). The marketing homepage uses the CSS-only homepage
// variant in shiny-button.tsx and ships zero framer-motion code,
// shaving JS payload from the largest TBT contributor on mobile.
// ──────────────────────────────────────────────
...
interface ShinyButtonConsoleProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  children: React.ReactNode;
  className?: string;
}

export default function ShinyButtonConsole({
  children,
  className,
  ...props
}: ShinyButtonConsoleProps) {
  return (
    <motion.button
