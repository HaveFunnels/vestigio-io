...
- **`impact_role: 'retention'` surface gap (full version).** Beyond the dashboard sum, the retention signal is the natural backbone of the Wave 21.3 monthly "value caught" report (it nearly doubles the perceived magnitude — "R$ Y retained + R$ X recovered" vs "R$ X recovered alone"). Surface points: workspace summary cards, FindingDetailPanel footer ("what this control is keeping you from losing"), monthly digest email, and the renewal-touch UI in `/app/billing` (right before plan-cycle-end, show retained value as part of the renewal CTA). Sanity check first: grep `impact_role` in `src/components/` to confirm where it's already wired vs missing.

- **`CompoundFinding` chains: collapse vs list in UI.** Wave 19d wired compound findings into the surface (commit 2f8fb79). The detection engine groups N findings under one `parent_chain_id`, but the FindingDetailPanel + the dashboard list likely still render each component finding as its own card. The grouping signal is lost — user sees 6 cards where they should see 1 expandable group. Fix: a `<CompoundCard>` wrapper that, when given findings with the same `parent_chain_id`, collapses to a single "5 findings caused by `trust_hesitation_revenue` chain" row that expands on click. Big signal/noise win at no engine cost.

- **`DomainFingerprint` (Wave 19c) reuse.** Snapshot is already written per first-audit (commit a2e46f7). Applications that cost zero new collection:
  - **Welcome-back signin:** "Last audit identified you as a Shopify Plus store with Stripe + Klaviyo — still accurate?" Confirm-or-update lets the engine adjust the businessModel inference up-front without waiting for the next audit cycle.
  - **Cycle-over-cycle fingerprint diff:** "You added Klaviyo this week. Reclassified your email-coverage tier." Surfaced as a `WhatChanged` widget on the dashboard.
  - **Anonymized marketing testimonial:** "Shopify Plus store + R$ 200k MRR resolved `policy_gap` in 3 weeks." Pulled from the BenchmarkSnapshot pipeline (see "Cross-customer inference benchmarks" above), filtered by fingerprint similarity.

- **`ContentEnrichmentCache` (Wave 19c) admin telemetry.** The cache is documented at ~$0.03/mo/env in `project_always_on_cost_analysis` memory but there's no admin surface showing hit-rate. Without that, the cost-optimization decision was made on assumption rather than measurement. Add `/app/admin/cache-stats` showing: total enrichments requested per day, cache hit % per enricher (7 enrichers per Wave 19c), $ saved vs the no-cache baseline. Justifies the cache investment for the team and surfaces drift if hit rate degrades.

- **Chronic-finding detection from CycleSnapshot.** `projectChangeReport` already runs and the `CycleSnapshot` table already exists. If a finding appears → resolves → reappears N times across cycles, it's CHRONIC (vs. transitory). Today the UI only surfaces the LATEST regression, not the pattern. Surface: "This problem came back for the 3rd time in 5 weeks" — direct narrative for the always-on (Wave 21) pitch ("you need continuous monitoring, here's why"). Query: `SELECT finding_key, COUNT(*) FROM finding_lifecycle_events WHERE event IN ('appeared','resolved') GROUP BY finding_key HAVING toggles > 4`.

- **Auto-generated "Findings of the Week" inbound editorial.** 177 inference keys + the rich `reasoning` text per pack + cross-customer aggregation. Weekly cron picks top 5 most-common inferences across all cycles in the past 7 days, takes the canonical reasoning fragment from the pack file, generates a public blog post via Haiku ("This week, 3.4M e-com sites hit `cart_variant_weak_control` — here's what that means and how to spot it"). Pairs naturally with the cross-customer benchmark pipeline above. Reusable assets: `packages/inference/packs/*.ts` reasoning text is already public-friendly prose. Output to `src/app/(content)/insights/[slug]/page.tsx`.

- **Slack notification channel for critical findings.** The Brevo-based daily digest (Wave 3.13) already exists. Adding Slack as a destination: customer connects workspace via OAuth, gets push when a finding with `severity=critical` lands. Same digest as email (item below) plus a real-time "finding just appeared" ping. Slack Bot Token + `/app/integrations/slack` connect flow + 1 webhook endpoint = ~1 day.

- **Email + Slack digest: combine retention + new-findings + chronic-detection in one report.** Three independent items above (`impact_role` surface, "Findings of the Week", Slack notif) collapse cleanly into ONE periodic digest pipeline:
  - **Weekly customer digest** (push to email + Slack): new findings this week, chronic patterns flagged, **retention value retained this period**, and a single "fix-first" recommendation.
  - **Monthly value-caught report** (the Wave 21.3 hook): same pipeline, longer cadence, retention/loss split prominent.
  - **Public Findings-of-the-Week post**: weekly cron, cross-customer aggregation.
  Same Brevo + Slack plumbing for all three; the only differentiator is the data window + audience filter. Build the digest infra once.

- **`ConfidenceNarrative` deferred until we can act on contested signals.** `buildConfidenceNarrative` ([packages/projections/engine.ts:1968](../packages/projections/engine.ts#L1968)) already threads through every workspace. Surfacing the raw narrative ("92% of signals harmonized, 3 contradictions resolved, 0 contested") is transparency but risks looking like noise without an action handle. Hold until we ship: an MCP chat handle where the user can ask "why is this contested?" and the engine returns the contradiction context (`getContradictionContext` already exists in `packages/truth/consistency-guard.ts`). At that point the badge becomes a CTA, not a disclosure.

- **MCP: deprecated `answer_X` tool measurement + sunset.** [`apps/mcp/tools.ts:488-499`](../apps/mcp/tools.ts#L488-L499) still dispatches `answer_can_i_scale` / `answer_where_losing_money` / `answer_fix_first` / `answer_underlying_cause` as compatibility aliases. The unified `answer_intent` tool is canonical (one tool, intent param). Playbooks still reference the deprecated names too (`apps/mcp/playbooks.ts:44-95`). Sunset path: (a) instrument dispatch to count `answer_X` calls by name, (b) migrate playbooks to `answer_intent` with proper intent params, (c) after 30 days of measurement, delete the dead alias cases.

- **MCP: surface `analyze_copy` guideline metadata to Claude.** Wave 3.10 shipped the copy pack with 80 guidelines KB. The `analyze_copy` tool today returns findings but NOT which guideline was violated — Claude sees "value_proposition_buried" but not "violates guideline #47 (specificity_deficit)". Wire the guideline reference into the tool response so Claude can cite specific rules in chat ("This CTA violates the 'one primary CTA per page' guideline from CRO best-practices"). The guidelines KB already exists.

- **MCP: `get_funnel_state` integration with `funnel_journey` pack.** The MCP tool `get_funnel_state` exists ([apps/mcp/tools.ts:211](../apps/mcp/tools.ts#L211)) but is independent of the `funnel_journey` orphan above. If `get_funnel_state` returns funnel-stage findings, that becomes the MCP-side surface for the funnel_journey pack data EVEN BEFORE we ship the workspace UI. Quick win: route funnel_journey findings into `get_funnel_state`'s response so the chat path unblocks before the UI ships.

- **Strategic — "always-on" needs a quantitative proof point BEFORE Wave 21 ships.** The inevitability thesis ([memory: project_inevitability_thesis](../.claude/projects/-Users-luisgall-Downloads-Vestigio-io-vestigio-io/memory/project_inevitability_thesis.md)) leans toward "always-on revenue protection". Wave 21 builds the mechanism but needs PROOF that always-on catches things weekly audits miss. Instrument NOW on existing customers: log "time between real-world breakage → audit cycle observed → customer took action". If the median latency exceeds 7 days, the always-on pitch sells itself when Wave 21 ships. No new collection — just add timestamps to existing finding lifecycle events.

- **Strategic — Ad spend ROI is the highest-dollar use case and needs better surfacing.** Wave 3.9 shipped ad-message-match. Wave 3.8 shipped Stripe. The JOIN — "Meta ad X drove R$ Y of GMV at Z% conversion rate vs site avg" — is the killer feature for the customers who burn R$ 50k+/mo in Meta. While Meta integration approval is pending (1-6 weeks per the roadmap manual-config table), build the **CSV upload path**: customer exports Facebook Ads CSV, uploads with date range, engine analyzes the JOIN with already-collected Stripe revenue data. This is the same engine path as the integrated version, just with manual data instead of API. Bridge revenue for customers who can't wait for Meta OAuth approval. Same approach generalizes to Google Ads CSV.

- **Perf items reassessed during execution (deferred with rationale):**
  - **`assertTruthResolved` already O(N) with minimum work.** The function does a property check `if (!sig.truth_metadata)` per signal — no Map lookup, no set hash. A "set bit-field" approach wouldn't be faster because the property check IS already O(1). The function throws on first miss (best-case fast), and only iterates fully in `'warn'` mode. The real perf win would be to AVOID calling the guard when all signals provably came from `harmonizeSignals` (which now attaches metadata) — but the guard's purpose is to catch BYPASS paths (saasSignals etc.), so removing it on the main path would defeat the safety design. No change needed.
  - **`makeRef('signal', sig.id)` caching.** 178 call sites across packs concatenating `'signal' + ':' + sig.id`. V8 already interns short string literals and the template-literal hot path is well-optimized in modern engines. Without a benchmark proving the allocation cost dominates, caching adds Map overhead per call that could be net-negative. Defer until profile data justifies the invasive change (touches every `createSignal` helper + every pack call site).

- **Strategic — expand the "Fix with AI" surface toward autonomous remediation.** Wave 3.13's "Fix with AI" is the embryo of the PostHog-Code positioning (per memory `project_posthog_code_positioning`). Today it generates a recommendation; the customer copies/applies manually. Expansion paths (graduated, easiest first):
  1. **Generate PR**: for findings on the customer's GitHub-tracked code (copy on landing page, CTA text, meta descriptions), the action drawer generates a real PR via GitHub App.
  2. **Apply via platform API**: for Shopify themes, the action drawer modifies the theme directly with explicit confirmation ("This will replace the headline in your homepage hero. Confirm?").
  3. **Email/copy drafts**: for marketing actions, generate a draft email or post that the customer just reviews + sends.
  Each step deepens the value capture from "Vestigio told me what to fix" to "Vestigio fixed it for me". Major repositioning when shipped — Wave 22+ candidate but worth scoping during the Wave 21 design.
