...
> Last updated: 2026-05-12 (inventory backend + frontend hardening shipped; residual waves 9.1–9.5 added)
...
| Inventory frontend — polish residuals | **Not started** — Residual from 2026-05-11 inventory frontend audit (sweep already shipped pagination, sort, CSV export, ARIA, real deltas, classification confidence, response time, drawer responsive). Remaining: (a) expose `classificationSignals` JSON in drawer — show which signals (regex/content/structure/LLM/business) voted for the resolved page type. (b) Discovery sources still use `titleCase()` instead of `t()`. (c) Two sources of truth for commercial page types — `COMMERCIAL_PAGE_TYPES` set in `/api/inventory/route.ts` vs `getPageTypeStyle()` in `lib/page-type-colors.ts`; pick one canonical. (d) `isDemoMode()` checks duplicated across 3+ files, consolidate into one helper. (e) 3s polling for cycle status (`/api/cycles/latest`) — verify if banner already uses the SSE stream we wired earlier and remove the timer if so. | Wave 9.4 |
