...
**Status**: 3c MCP (`1c8a8681`) ✅ · 3b plano (`8c7c7880`/`77d6e0e7`) ✅ · 3a competitive (`3f456889`) ✅ · lista de superfícies no inventário (`339991f2`) ✅. **Falta**: chip de purpose no finding/action drawer (adiado — esses componentes não têm `envId`; precisa API leve `/api/business-context` + fetch), ranking tiebreaker + dispatch `vertical-inference` (threadar `getBusinessContext` no `recompute`→`projectAll`, caminho quente), `analyze_copy` por purpose (marginal — 3c já dá surfaces ao copilot). Risco label-errado **assumido**. **Nota de design**: não reescrever `INFERENCE_TITLES` (ecommerce-específicos); só labels de superfície *exibidos* recebem purpose. Observar via `SELECT perceivedVertical,perceivedSurfacesJson FROM "Environment"` após um ciclo.
