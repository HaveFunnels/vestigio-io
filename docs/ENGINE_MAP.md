...
|---|--------|----------|--------|
| 1 | `computeTripleSourceInferences` — 7 inference types | `packages/inference/triple-source-inference.ts:100` | **Exported from `inference/index.ts:8`, zero call sites.** Comment in `cross-domain-inference.ts:26-29` says it was absorbed into the heuristic fallback, but the original file was never deleted. |
