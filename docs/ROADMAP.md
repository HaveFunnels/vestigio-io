...
| ~~Unified Entity Architecture~~ | **✅ Fases 1-2 shipped.** Fase 1 (2026-04-21): FindingDetailPanel, cross-refs, canonical URL, filter state. **Fase 2 (2026-05-01):** SavedView Prisma model, 4 default views (Pegando Fogo/Aguardando Verificação/Por Domínio/Funcionando) with Phosphor icons + colored tabs, ViewSelector component, `/app/findings` page with view switching + groupBy rendering, sidebar simplification (Analysis→Findings+Inventory top-level), `/app/analysis` redirect, SaveViewModal. **Fase 3 (custom views/pin) deferred.** | Wave 3.20 |
