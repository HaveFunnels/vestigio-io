 ...
 * needs the planId) — that still happens later. Returns false for
 * tokens that are syntactically broken or past expiry; true means
...
export function isExportTokenWellFormed(token: string): boolean {
	...
	const expiresAt = parseInt(parts[0], 10);
	if (!Number.isFinite(expiresAt) || expiresAt < Date.now()) return false;
	if (!/^[0-9a-f]+$/i.test(parts[1])) return false;
	return true;
}
