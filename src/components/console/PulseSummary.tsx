...
export default function PulseSummary({
  ...
  if (loading) {
    return (
      <div className="relative flex overflow-hidden border-l-2 border-emerald-500/40 bg-emerald-500/[0.02]">
        <div className="flex-1 px-5 py-4">
          <div className="mb-2.5 flex items-center gap-2">
            <div className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />
            <span className="font-[family-name:var(--font-jetbrains-mono)] text-[10px] font-medium uppercase tracking-[0.15em] text-emerald-400/70">
              Vestigio Pulse
            </span>
          </div>
          <div className="space-y-1.5">
            <div className="h-3 w-full animate-pulse rounded-sm bg-white/[0.04]" />
            <div className="h-3 w-[80%] animate-pulse rounded-sm bg-white/[0.04]" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative flex overflow-hidden border-l-2 border-emerald-500 bg-emerald-500/[0.03]">
      {/* Subtle scan line */}
      <div
      ...
      <div className="relative flex-1 px-5 py-4">
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className="font-[family-name:var(--font-jetbrains-mono)] text-[10px] text-emerald-400/30">
            {new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
          </span>
        </div>
        <p
          ref={textRef}
          className={`text-[13px] leading-[1.7] text-zinc-300 transition-opacity duration-700 ${
