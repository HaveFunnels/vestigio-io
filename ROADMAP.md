...
**Status**: 3c MCP (`1c8a8681`) ✅ · 3b plano tese+narrativa+next-steps (`8c7c7880`/`77d6e0e7`) ✅ · 3a competitive (`3f456889`) ✅ · **falta**: lista de superfícies no drawer "O que analisamos" + label de superfície percebida no finding/action drawer (UI, visível, render não-verificável daqui), ranking tiebreaker + dispatch do `vertical-inference` (precisa threadar `getBusinessContext` no recompute→`projectAll`), `analyze_copy` por purpose (marginal — 3c já dá surfaces ao copilot). Risco de label-errado **assumido pelo fundador** (sem gate de observação). **Nota de design (importante)**: NÃO reescrever o texto estático dos findings (`INFERENCE_TITLES`) — muitos são ecommerce-específicos ("checkout" no título); só o LABEL de superfície *exibido* recebe o purpose percebido ("/agendar" → "página de agendamento"). Observar via `SELECT perceivedVertical,perceivedSurfacesJson FROM "Environment"` após um ciclo.
