/**
 * Vestigio Behavioral Intelligence Snippet v2.6
 *
 * v2.6 (2026-09-09): confirmation hardening — a lone title/H1 word match
 * no longer asserts a completed purchase on a transaction surface
 * (checkout/cart/payment/order). A checkout greeting ("Bem-vindo") or an
 * order page shown before a PIX payment settles can no longer fabricate
 * a conversion; the merchant's window.vestigio.confirm({order_id}) stays
 * the robust path there. Weak signals still catch generic thank-you
 * pages off a transaction surface.
 *
...
(function() {
  ...
  // Commerce transaction surfaces — checkout, cart, payment, and order
  // pages. These are exactly where a pre-payment greeting or label
  // ("Bem-vindo", "Welcome", an order that exists but is unpaid) lives,
  // so a lone title/H1 word match here must NOT be trusted to assert a
  // completed purchase. `/c/` is the common short checkout-slug form.
  var TRANSACTION_PATH_PATTERNS = /\/(c|checkout|checkouts|cart|carrinho|carrito|pagamento|payment|pago|finalizar|billing|order|orders|pedido|pedidos)(\/|$|\?|#)/i;
  ...
  function checkConfirmation() {
    ...
    // Signal strength. A dedicated confirmation URL (url_pattern) or an
    // explicit paid DOM marker (dom_marker) is STRONG — it asserts a
    // completed purchase on its own. A title/H1 word match is WEAK: it
    // catches thank-you pages whose URL is generic, but it also fires on
    // a checkout greeting ("Bem-vindo") or an order page shown before a
    // PIX payment settles (~35% never do). So a weak signal alone counts
    // only OFF a transaction surface. On checkout/cart/payment/order
    // pages the robust path is the merchant calling
    // window.vestigio.confirm({order_id}) — which does not go through
    // here at all — never our guess at a heading.
    var strong = signals.indexOf('url_pattern') !== -1 || signals.indexOf('dom_marker') !== -1;
    var onTransactionSurface = TRANSACTION_PATH_PATTERNS.test(pathname);
    if (signals.length > 0 && (strong || !onTransactionSurface)) {
