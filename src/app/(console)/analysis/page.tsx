...
function AnalysisContent({
  ...
  const columns: Column<FindingProjection>[] = [
    ...
    },
    {
      ...
      render: (row) => row.polarity === 'positive'
        ? <span className="text-xs font-mono text-emerald-600 dark:text-emerald-400">+{row.impact.midpoint >= 1000 ? `$${(row.impact.midpoint / 1000).toFixed(1)}k` : `$${row.impact.midpoint}`}{tc("per_month_short")}</span>
