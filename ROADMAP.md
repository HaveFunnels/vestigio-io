...
**Status**: 3c (MCP context, `1c8a8681`) ✅ · 3b (tese+narrativa+next-steps, `8c7c7880`/`77d6e0e7`) ✅ · **falta**: ancoragem do texto do finding (3a, delicado — catálogo estático), lista de superfícies no drawer "O que analisamos" (UI, visível ao cliente), swap competitive `resolveCustomerType` (precisa mapa `PerceivedVertical`→`CustomerType`, não é one-liner), ranking tiebreaker + dispatch do `vertical-inference`, `analyze_copy` por purpose. **Gate**: rodar 1 ciclo da havefunnels e validar a percepção antes dos wires visíveis ao cliente (lista de superfícies, ancoragem de finding).
