    ...
    return (
      <div className="mt-2 space-y-2">
        <textarea
          ...
          className="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-emerald-600"
          rows={3}
          autoFocus
        />
        <div className="flex gap-2">
          ...
          <button onClick={() => { setEditing(false); setEditText(content); }} className="rounded-md border border-zinc-700 px-3 py-1 text-xs text-zinc-400 hover:text-zinc-300">Cancel</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
        {/* Copy */}
        <button onClick={handleCopy} className="rounded p-1 text-zinc-600 hover:bg-zinc-800 hover:text-zinc-400" title="Copy">
          {copied ? (
        ...
        {/* Edit (user only) */}
        {role === "user" && onEdit && (
          <button onClick={() => setEditing(true)} className="rounded p-1 text-zinc-600 hover:bg-zinc-800 hover:text-zinc-400" title="Edit & resend">
        ...
        {role === "assistant" && onRetry && (
          <button onClick={onRetry} className="rounded p-1 text-zinc-600 hover:bg-zinc-800 hover:text-zinc-400" title="Retry">
        ...
        {role === "assistant" && onFeedback && (
          <>
            ...
            <button
              ...
              className={`rounded p-1 ${feedback === "positive" ? "text-emerald-500" : "text-zinc-600 hover:bg-zinc-800 hover:text-zinc-400"}`}
              title="Good response"
            >
            ...
            <button
              ...
              className={`rounded p-1 ${feedback === "negative" ? "text-red-500" : "text-zinc-600 hover:bg-zinc-800 hover:text-zinc-400"}`}
              title="Poor response"
            >
        ...
        <div className="mt-2 animate-in slide-in-from-bottom-2 duration-200">
          <div className="rounded-lg border border-zinc-700 bg-zinc-900 p-3">
            ...
            <textarea
              ...
              className="mt-1.5 w-full resize-none rounded-md border border-zinc-800 bg-zinc-950 px-2.5 py-1.5 text-xs text-zinc-300 placeholder-zinc-600 outline-none focus:border-zinc-700"
              rows={2}
              autoFocus
            />
            <div className="mt-2 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-md border border-zinc-700 px-2.5 py-1 text-[11px] text-zinc-400 hover:text-zinc-300"
                >
                  Skip
                </button>
                <button
