# COMPLETED_ROADMAP.md — Vestigio Completed Work Archive

> Extracted from ROADMAP.md on 2026-04-17 to keep the active roadmap lean.
> For the active roadmap with open items, see [ROADMAP.md](ROADMAP.md).
...
> Companion to: [NORTHSTAR.md](NORTHSTAR.md), [DEV_PROGRESS.md](archive/DEV_PROGRESS.md), [FINDINGS_OPPORTUNITIES.md](FINDINGS_OPPORTUNITIES.md), [COLLECT_OPPORTUNITIES.md](COLLECT_OPPORTUNITIES.md)
>
> **2026-04-27 Waves 3.9 C-E, 3.11B Fase 5, 3.12, 3.15, 3.17 shipped + Shopify bug fix:** Major sprint completing 5 roadmap items. **(1) Shopify promoted product bug fix (3.7.2-I).** Added `handle` field to `ShopifyProduct` interface + product fetch query. Poller now cross-references product handles with crawled `/products/{handle}` URLs to populate `promotedProductIds`. Finding M (`promoted_product_out_of_stock`) now fires when promoted products are out of stock. **(2) 3.9 C-E — Ad Creative→LP Message-Match pipeline.** New `workers/ingestion/enrichment/ad-message-match.ts` module: `extractAdLpPairs()` traverses graph `ad_creative` nodes + `ad_targets` edges and pairs with page copy evidence. `analyzeAdMessageMatch()` sends each pair to Haiku for structured assessment (headline match, CTA match, value prop consistency, specific mismatch points). `parseAssessment()` extracts structured data. New evidence type `ad_message_match`, signal `ad_message_mismatch_detected` in signal engine, inference `inferAdCreativeMessageMismatch` aggregating total ad spend across mismatched pairs. Root cause `ad_landing_promise_gap`, impact baselines (high 4-7%, medium 2-4%, low 1-2%), 5-step remediation catalog. Wired in `run-cycle.ts` between integration polling and `recomputeAll`. **(3) 3.11B Fase 5 — Workspace enrichment i18n.** All workspace enrichment components (TrustScoreCard, ResilienceChecklist, ChargebackResilience, SecurityPosture, OpportunityPreview, CrossSignalHero, ActivityHeatmap) converted from hardcoded strings to useTranslations(). ~50 new dictionary keys across 4 languages. **(4) 3.15 — Cross-Signal Surface.** Dedicated `/app/cross-signals` page with `CrossSignalsShell` (hero stats: total chains, combined impact, perspectives involved; filters by perspective + temporal pattern). `GET /api/cross-signals` API endpoint. `CrossSignalChainCard` expandable component with link flow visualization, narrative text, temporal badge (sequential/simultaneous). Aggregator refactored with shared `buildCrossSignalChains()` helper, temporal pattern detection via cycleId comparison (same cycle = simultaneous, different cycles = sequential), `computeAllCrossSignals()` export. Template-based narrative generator at `src/lib/dashboard/cross-signal-narrative.ts`. Sidebar nav entry under Analysis group. Mock data for demo environments. CrossSignalHero dashboard widget enhanced with temporal patterns + narratives. i18n (4 langs). **(5) 3.17 — Upgrade & Feedback Moments.** Scope expanded beyond original spec. Upgrade side: `PlanProvider` + `usePlan()` hook (Starter/Pro/Max resolution from org.plan), `UpgradeNudge` component (3 variants: inline, badge, blurred-overlay), copilot Pro pill badge for Starter users, copilot upgrade nudge in empty state, `WhatChangedCard` cadence nudge ("Upgrade to Pro for daily insights"), `CrossSignalHero` chain limiting (Starter: 1 chain + "N more with Pro"), `AdSpendKpi` blurred preview overlay for Starter. Feedback side: `FeedbackMoment` component (2 variants: rating 5-star, NPS 0-10), `useFeedbackMoment` hook with 3-layer cooldown (48h per-trigger localStorage, session global, 3-dismiss suppression), `NpsPulse` 14-day pulse at bottom-left z-44, copilot feedback after 3 messages, `FindingDetailPanel` 10s dwell timer trigger, `/api/feedback` extended with "contextual" + "nps" types + 0-10 rating range. i18n (4 langs). **(6) 3.12 — Unified Impact-Ranked Actions Pipeline (all 6 fases).** Engine: 5 new fields on `ActionProjection` (uplift_hypothesis, upside_score, value_case_basis, cluster_key, cluster_count), opportunity data resolved from `MultiPackResult.opportunities` + cluster data from `opportunity_compression` in projection engine, auto-verify logic (change_class === 'improvement' + status === 'implemented' → 'verified'). Frontend: 6 tabs simplified to 2 (Pipeline/My Actions) + filter bar (type/severity/effort/status), 4 unified summary cards (Total Exposure dual-tone, Quick Wins, In Progress, Captured), type+upside emerald badges, hypothesis inline in table rows, hypothesis emerald card in ActionDrawerContent with upside score + effort hint + basis + cluster info, list/scatter view toggle. `ScatterPlot.tsx` SVG component (effort × impact, 4 quadrants: Quick Wins/Big Bets/Fill-ins/Strategic, red dots for incidents, emerald for opportunities). Platform: `OpportunityTracking` Prisma model (actionKey + environmentId unique, status field), `PATCH /api/actions/[id]/status` endpoint with transition validation (identified→accepted→implemented→verified, with dismiss/reopen paths). i18n (4 langs, ~25 keys). **(7) Quick wins (copilot routing + i18n + ES/DE translations).** All `router.push('/chat?...')` calls across 5 pages replaced with `copilot.open()` method. Hardcoded English strings in workspace/dashboard components (TrustScoreCard, ResilienceChecklist, ChargebackResilience, SecurityPosture, OpportunityPreview, CrossSignalHero tooltip, ActivityHeatmap tooltip) converted to i18n. `reasoning_slots` templates added for ES/DE (inference_causes, inference_effects, reasoning_templates). Full ES + DE dictionaries completed.
>
> **2026-04-14 Wave 5 Fase 3 shipped — continuous incremental engine:** The three pieces that make the "continuous intelligence" pitch real landed together. **(1) Real `cycleType` branching in the audit-runner.** `apps/audit-runner/cycle-modes.ts` owns the hot/warm/cold config (behavioral window hours, min sessions for inferences, wall-clock budget, carry-forward flag) and the primitives: `isCriticalSurfaceUrl` (regex over checkout/pricing/product + home), `resolveCriticalSurfaces` (heuristic ∪ URLs that had severity>=high findings in the last 7d — automatic promotion of surfaces actively breaking revenue), `buildUrlAllowList` (hot = critical only, warm = critical + 30% rotating random sample, cold = null/all), `carryEvidenceForward` (copies Evidence rows from the previous completed cycle to the new cycleRef so hot/warm cycles don't lose continuity for pages they don't re-parse). `apps/audit-runner/run-cycle.ts` now maps `cycle.cycleType` (previously cosmetic) to a `CycleMode`, resolves the previous cycle + critical set + allow-list, calls `carryEvidenceForward` for allow-listed URLs, then runs the staged pipeline with `url_filter` and a per-mode `global_timeout_ms`. Hot/warm cycles use `pipelineMode: 'shallow_plus'` which skips Stage D (Playwright) — browser verification stays cold-only because it's the most expensive stage and only pays off in baseline context. **(2) Content hash persistence.** `Evidence.contentHash` column + index (`@@index([environmentRef, subjectRef, evidenceType])`). The hash was already computed in `staged-pipeline.ts:358` for loop detection; now it's threaded through `addHttpEvidence` → `createEvidence` → `PrismaEvidenceStore` into Postgres. Enables future-Wave work to pre-check hash before dispatching the fetch and skip the fetch entirely for unchanged pages. **(3) Behavioral window parameterization.** `processBehavioralEventsForEnv` now accepts `windowHours` — hot uses 1h, warm uses 24h, cold holds the legacy 30d baseline. Paired with `CYCLE_MODE_CONFIG.minSessionsForInferences` (hot=5, warm=10, cold=20) so hot cycles can still surface fresh friction without the full cohort statistics that only make sense with the 30d window. **(4) Plan cadence + scheduler cron.** `src/libs/plan-config.ts` gets `PLAN_CADENCE` (Starter: cold 1/week only; Pro: hot 1h + warm 4h + cold 3d; Max: hot 15min + warm 1h + cold 1d). `getCadenceForPlan(planKey)` resolves it. New `apps/audit-runner/scheduler.ts` runs hourly from `instrumentation-node.ts` under `withLeadership("audit-scheduler", ttlSec:90)` (multi-replica safe). `runSchedulerPass()` enumerates every `activated=true, continuousPaused=false, organization.status != suspended` env, calls `resolveDueCycleType(envId, planKey)` which reads last-completed timestamps per cycleType (legacy "full" rows count as cold for freshness), skips envs with in-flight cycles (pending/running), and picks the highest-priority overdue tier (cold > warm > hot). Creates the `AuditCycle` row and calls `enqueueAuditCycle()` with the corresponding priority, with in-process fallback when Redis isn't configured. Demo orgs are NOT excluded from the scheduler (unlike the inactivity-pause cron) so the demo account stays refreshed even without real user access. **Staged pipeline change:** accepts new optional `url_filter: string[]` in `StagedPipelineInput`. When present, the Stage C crawl loop intersects discovered candidates with the allow-list (homepage is always retained regardless). Cold cycles pass undefined → pipeline crawls everything the discovery finds. **Not yet in this ship** (deliberately scoped out): `FindingEvidenceDep` index (engine recompute remains atomic over the full evidence set — hot/warm cycles get their speedup from crawl selectivity + carry-forward instead), pre-fetch hash comparison to skip the fetch entirely (would require either HEAD+body-reconciliation or a hash-only endpoint; today we carry forward evidence but still fetch all allow-listed URLs — enough win for Fase 3 MVP), user-configurable critical surface marking UI (confirmed out of scope in this session). Build clean; zero behavioral changes for legacy cycles that still carry `cycleType="full"` (they flow through the cold path, which is unchanged from pre-Fase-3 behavior).
>
> **2026-04-14 Wave 5 Fase 2 + Fase 1 shipped — owner-driven activation flow + scale-ready audit infra:** Two full ships in this session on top of admin provisioning. **(1) Fase 2 — owner-driven activation flow (commit `847a88b`).** Schema migration adds `Environment.activated`, `Environment.lastAccessedAt`, `Environment.continuousPaused`. New `POST /api/environments/activate` creates env + BusinessProfile + dispatches the first AuditCycle (cycleType=full) and flips `activated=true`. JWT now carries `hasActivatedEnv` and the middleware routes shell-org members through `/app/onboarding` until the env is activated. Onboarding refactor pulls the env+profile creation out of `/api/onboard` (still used by self-serve Paddle path) and into the new endpoint, with the "Activate environment" CTA replacing the Paddle button when the org already has a plan. SSE wiring lands: new `GET /api/cycles/[id]/stream` (pure observer — polls status + finding count + pages every 2s, emits status/complete/error events with 15s heartbeat + 10min guardrail) and `GET /api/cycles/latest` for banner discovery. New `CycleProgressBanner` lives in `AppSidebarLayout` scoped to `/app/inventory`, `/app/analysis`, `/app/actions`. Activity tracking via `src/libs/env-activity.ts` (1h-debounced `lastAccessedAt` write + auto-resume on access that dispatches a catch-up cycle) called from the `/app/*` server layout. Hourly inactivity-pause cron in `src/instrumentation-node.ts` pauses envs with no access for 14d (excluding `orgType=demo`) and writes a `NotificationLog` event for the downstream mailer. Subdued amber paused-banner shows on `/app/*` when any env is paused. **(2) Fase 1 — scale-ready foundation infra (this commit).** New `apps/platform/audit-cycle-queue.ts` is a Redis-backed priority queue with three tiers (hot > warm > cold), per-env locking via `SET NX EX`, attempt counting, and a DLQ for cycles that fail 3+ times. New `apps/audit-runner/worker-loop.ts` is a standalone consumer process that drains the queue with per-worker concurrency cap (default 2, env-tunable via `AUDIT_WORKER_CONCURRENCY`), env-contention requeue (no retry-counter penalty), exponential-backoff retry (5s → 60s capped), graceful SIGTERM drain (5min timeout), and a tiny HTTP health server on `WORKER_HEALTH_PORT` (default 3001) returning queue depth + chromium pool stats + Redis status for Railway probes. New `npm run start:worker` script + tsx as runtime dep. Stripe + Paddle webhooks + `/api/environments/activate` + the env-activity resume hook + the orphan-redispatch heal cron all refactored to `enqueueAuditCycle()` first, with the legacy `Promise.then(runAuditCycle)` retained as fallback when Redis isn't configured (single-box deploys still work). New `src/libs/leader-election.ts` (`SET NX EX` based, fail-open on Redis blip) wraps the heal, lead-cleanup, and inactivity-pause crons so multi-replica Railway deploys don't N-fold the work. New `workers/verification/chromium-pool.ts` provides an in-process semaphore (default 3, env-tunable via `CHROMIUM_POOL_SIZE`) wrapping every `chromium.launch()` call site (`playwright-runtime.ts` + `authenticated-runtime.ts`) so a burst can't OOM the worker — caps RAM around 1GB. New `src/libs/structured-log.ts` emits JSON lines with cycle_id/org_id/env_id/worker_id correlation IDs (snake_case keys for log aggregator parsing). Pay-as-you-go meter foundation: `src/libs/usage-meter.ts` records `cycles_run` + `pages_crawled` + `compute_seconds` to the existing `Usage` table (period bucket `YYYY-MM`); hook in `runAuditCycle` finally{} block. Admin metrics endpoint at `GET /api/admin/metrics/audit-runner` returns queue depth (per tier + DLQ), cycles-by-status last-24h, p50/p95 cycle duration, recent failures (20), top-10 orgs by cycles_run this period — single call cheap enough for a 30s dashboard refresh. Worker validated: boots with structured logs, registers SIGTERM handler, opens health endpoint, drains gracefully. **Net result:** a freshly-impersonated owner can complete onboarding → first audit fires through the priority queue → worker drains it → SSE banner shows live progress → findings populate → idle for 14d → auto-paused → returns → catch-up cycle dispatched. The infra under it can scale to N worker replicas + M web replicas safely (queue is FIFO-atomic, locks are per-env, leader election prevents cron duplication, chromium pool prevents OOM, metrics tile makes saturation visible). Wave 5 Fase 3 (real `cycleType` branching + scheduler cron + content-hash skip) builds on top of this foundation.
>
...
> code, not assumptions). See [DEV_PROGRESS.md § Pipeline Audit 2026-04-06](archive/DEV_PROGRESS.md#pipeline-audit--2026-04-06--ground-truth-vs-roadmap)
...
See [DEV_PROGRESS.md](archive/DEV_PROGRESS.md) for the full build history. Key milestones:
...
| Shopify promoted product bug fix (3.7.2-I) | ✅ **Done — 2026-04-27** | Wave 3.7 | `handle` field on ShopifyProduct, poller cross-refs handles with crawled URLs, `promotedProductIds` populated, finding M fires |
| Ad Creative→LP message-match pipeline (3.9 C-E) | ✅ **Done — 2026-04-27** | Wave 3.9 | `ad-message-match.ts` module, Haiku analysis, signal + inference + root cause + impact + remediation, run-cycle wiring |
| Workspace Lens Enrichment i18n (3.11B Fase 5) | ✅ **Done — 2026-04-27** | Wave 3.11B | ~50 keys across 4 languages for workspace enrichment components |
| Unified Impact-Ranked Actions Pipeline (3.12) | ✅ **Done — 2026-04-27** | Wave 3.12 | All 6 fases: engine enrichment, unified list, filter bar, summary cards, scatter plot, status workflow, i18n |
| Cross-Signal Surface (3.15) | ✅ **Done — 2026-04-27** | Wave 3.15 | Dedicated page + API + shell + chain cards + temporal detection + narrative generator + sidebar nav + i18n |
| Upgrade & Feedback Moments (3.17) | ✅ **Done — 2026-04-27** | Wave 3.17 | PlanProvider, UpgradeNudge (3 variants), FeedbackMoment (rating + NPS), useFeedbackMoment cooldown hook, NpsPulse, copilot Pro badge, i18n |
| Quick wins: copilot routing + i18n + ES/DE translations | ✅ **Done — 2026-04-27** | Wave 3 | router.push→copilot.open across 5 pages, hardcoded strings i18n'd, reasoning_slots ES/DE |
| Re-engagement & Remediation complete (3.13) | ✅ **Done — 2026-04-27** | Wave 3.13 | Fases 1-4 complete: dashboard landing, CrossSignalHero, daily digest, Fix with AI (shared component, FindingDetailPanel multi-action picker), i18n |
| First-Audit Experience (3.18) | ✅ **Done — 2026-04-27** | Wave 3.18 | FirstAuditProgress (5-stage SSE-driven timeline + heuristic preview), FirstAuditCelebration (emerald glow dots), DashboardShell wiring, i18n |
| Cancel Flow & Save Offers (3.19) | ✅ **Done — 2026-04-27** | Wave 3.19 | CancelSurvey model, 3-step cancel page, dynamic save offers by reason, Paddle API (pause/cancel/discount), win-back email, settings CTA, i18n |
| Stripe Integration (3.8) | ✅ **Done — 2026-04-27** | Wave 3.8 | OAuth Connect flow (authorize + callback), revenue poller (charges, subscriptions, MRR, churn, disputes, refunds, failed payments), sync route, run-cycle wiring, Data Sources UI, credential verification |
| Workspace Redesign complete (3.11) | ✅ **Done — 2026-04-27** | Wave 3.11 | TrendSparkline (SVG, auto-color), action count badges (bidirectional workspace↔actions links), dead-code annotations. All layouts/lenses/enrichment already complete from prior sessions |
| Copy Analysis Pack (3.10) | ✅ **Done — 2026-04-28** | Wave 3.10 | All 4 fases. Fase 1: Guidelines KB (80 guidelines from copywriting/CRO/marketing-psychology, 1171 lines), CopyElementsPayload extractor (399 lines), ICP fields on BusinessProfile, 8 new Haiku enrichment types (homepage_hero, social_proof_placement, objection_handling, urgency_scarcity, onboarding_copy, error_page_recovery, navigation_clarity, above_fold_density), 13 new signals, 8 new inferences, 7 root causes replacing single `copy_strategy_gap`, impact baselines + remediation catalog. Fase 2: `copy_alignment_pack` decision, CopyAlignment workspace component, `analyze_copy` MCP tool, `copy_audit` playbook (5 prompts), i18n (4 langs). Fase 3: ad-message-match integration (J), cross-page narrative consistency analysis (K), pricing page psychology with mental models (L). Fase 4: localization quality comparison (M), micro-copy audit (N), SEO-conversion tension detection (O), copy staleness regex detector (P, zero LLM cost). Total: 17 enrichment types, 20+ signals, 12+ inferences, 7 root causes, ~3000 new lines. |
...
| `cycleType` real branching (hot/warm/cold + critical-surface + carry-forward) | ✅ **Done — Wave 5 Fase 3 (2026-04-14)** | Wave 5 | `apps/audit-runner/cycle-modes.ts` + integration in `run-cycle.ts`; staged pipeline accepts `url_filter`; Evidence.contentHash persisted |
| `continuousAudits` scheduler (hourly leader-elected, per-plan cadence) | ✅ **Done — Wave 5 Fase 3 (2026-04-14)** | Wave 5 | `apps/audit-runner/scheduler.ts` + `PLAN_CADENCE` in plan-config; hot > warm > cold priority; demo orgs included |
| Audit-runner persistence (queue + worker service + leader election) | ✅ **Done — Wave 5 Fase 1 (2026-04-14)** | Wave 5 | `apps/platform/audit-cycle-queue.ts` + `apps/audit-runner/worker-loop.ts` + `src/libs/leader-election.ts`. Stripe/Paddle/activate/heal all enqueue first, in-process fallback retained for Redis-less deploys. |
| Chromium semaphore + structured logs + worker health endpoint | ✅ **Done — Wave 5 Fase 1 (2026-04-14)** | Wave 5 | `workers/verification/chromium-pool.ts` (semaphore of 3 default, env-tunable) wrapping every `chromium.launch()`; `src/libs/structured-log.ts` JSON lines with correlation IDs; `WORKER_HEALTH_PORT` HTTP endpoint with queue depth + pool stats |
| Pay-as-you-go usage meter + admin metrics endpoint | ✅ **Done — Wave 5 Fase 1 (2026-04-14)** | Wave 5 | `src/libs/usage-meter.ts` records cycles_run + pages_crawled + compute_seconds to existing `Usage` table; hook in `runAuditCycle` finally; `GET /api/admin/metrics/audit-runner` returns queue depth + p50/p95 + top orgs |
| Activation flow (owner-driven env creation + first-cycle trigger + SSE progress on inventory/analysis/actions + inactivity pause after 14d) | ✅ **Done — Wave 5 Fase 2 (2026-04-14)** | Wave 5 | commit `847a88b`. See § Wave 5 below. |

---

## Wave 0 — Critical Pipeline Gaps ✅ (7/7)
...
| **Acceptance** | ✅ Met. Snippet → endpoint → DB. See [DEV_PROGRESS.md § Wave 0.2](archive/DEV_PROGRESS.md). |
...
| **Acceptance** | ✅ Met. Snippet → ingest → audit cycle → behavioral findings + 7 workspaces light up. See [DEV_PROGRESS.md § Wave 0.3](archive/DEV_PROGRESS.md). |
...
| **Acceptance** | ✅ Met. Click "Re-verify" → real verification runs → drawer updates inline. See [DEV_PROGRESS.md § Wave 0.6](archive/DEV_PROGRESS.md). |
...
## Wave 1 — Core Experience Polish ✅ (9/9)
...
## Wave 2 — Knowledge, Members & Confidence (completed items)
...
**Verified:** 65/65 tests pass, 0 TS errors. See [DEV_PROGRESS.md § Wave 2.3](archive/DEV_PROGRESS.md) for the full diff.
...
**Verified:** 65/65 tests pass, 0 TS errors. Initial threshold attempt was 75/50 — broke 4 tests because fixtures produced findings in the 30-33 range. Lowering to 70/40 (aligned with the engine's existing internal floor) fixed it. See [DEV_PROGRESS.md § Wave 2.4](archive/DEV_PROGRESS.md) for the full diff.

---

## Marketing Surface Polish ✅

Already documented in the changelog (2026-04-10/11 and 2026-04-12 entries above).
...
Already documented in the changelog (2026-04-11 entry above).
...
## Wave 5 — Continuous Incremental Engine (Fases 1-3 ✅)
